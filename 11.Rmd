...
* Comapre results of everything
* m11.2
* m11.3
* m11.4 produces slightly different results
* m11.5
...
My results for model `m11.4` are somewhat different from text. Signs and magnitudes are similar. Not sure why.
...
My results differ from the book, but the `ulam` replication code also seems to produce the same results as me.
...
'
m_binomial <- stan(model_code = stan_program, data = stan_data)
...
'
m_poisson <- stan(model_code = stan_program, data = stan_data)
```
